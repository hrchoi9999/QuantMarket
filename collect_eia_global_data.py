...
from quantmarket_market.eia_collector import DEFAULT_EIA_API_KEY_PATH, collect_eia_series, eia_key_available  # noqa: E402
from quantmarket_market.global_context_db import DEFAULT_GLOBAL_CONTEXT_DB_PATH  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect EIA energy market series.")
    parser.add_argument("--db-path", default=str(DEFAULT_GLOBAL_CONTEXT_DB_PATH))
    parser.add_argument("--api-key-path", default=str(DEFAULT_EIA_API_KEY_PATH))
...
def main() -> None:
    ...
    if not eia_key_available(Path(args.api_key_path)):
        result = {
            "status": "skipped",
            "reason": f"EIA API key not found: {args.api_key_path}",
            "db_path": str(db_path),
            "coverage": [],
        }
        (report_dir / "eia_collection_latest.json").write_text(
            json.dumps(result, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return
    result = collect_eia_series(
        db_path=db_path,
