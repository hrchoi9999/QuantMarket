...
def _primary_forecast_contract(handoff_dir: Path, expected_asof: str | None = None) -> dict:
...
def _refresh_handoff(generated_at: str, expected_asof: str | None = None) -> dict:
    source_dir = ROOT / "service_platform" / "ai_training" / "market_context" / "current"
    handoff_dir = ROOT / "service_platform" / "quant_model_handoff" / "market_context" / "current"
    ...
    primary_contract = _primary_forecast_contract(handoff_dir, expected_asof=expected_asof)
...
def main() -> None:
    ...
    handoff = _refresh_handoff(generated_at=_now_kst(), expected_asof=args.expected_asof)
